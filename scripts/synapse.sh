...
# This allows the script to work even if you run it from ~/.local/bin
...
if [[ "$1" == "--install" ]]; then
    echo "➜ synapse: Requesting installation to $HOME/.local/bin..."
    mkdir -p "$HOME/.local/bin"
    cp "$0" "$BIN_PATH"
    chmod +x "$BIN_PATH"
    echo "✓ Done. You can now run 'synapse' from anywhere."
    exit 0
fi
