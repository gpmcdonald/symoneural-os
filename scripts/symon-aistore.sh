...
STORE_LABEL="${STORE_LABEL:-symon_store}"
...
deploy_aistore() {
  ...
  printf -v AIS_FS_PATHS '"%s": "%s"' "$STORE_MOUNT" "$STORE_LABEL"
  export AIS_FS_PATHS
  ...
  (
    ...
    # target count, proxy count, extra autogenerated mountpaths.
    make kill clean cli aisloader deploy <<< "$(printf '%s\n%s\n%s\n' "$target_count" "$proxy_count" "$extra_mountpaths")"
...
stage_download() {
  ...
  [[ -n "$url" ]] || fail "Usage: $SCRIPT_NAME download <url> [output-name]"
...
Usage:
  $SCRIPT_NAME check
  $SCRIPT_NAME deploy
  $SCRIPT_NAME download <url> [output-name]
